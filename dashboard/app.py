...
BUNDLE_IMAGE = os.environ.get("BUNDLE_IMAGE", "danbu/demoscale:demoscale-bundle-task-1.2.9")
...
.kind-banner strong{display:block}.kind-banner p{margin:.3rem 0}.kind-setup[hidden]{display:none}.kind-command-row{align-items:stretch;display:flex;gap:.5rem;margin-top:.7rem}.kind-command{background:#fff;border:1px solid #d4a653;border-radius:8px;color:#503000;display:block;flex:1;font:650 .78rem/1.45 ui-monospace,SFMono-Regular,Menlo,monospace;overflow-wrap:anywhere;padding:.65rem .75rem;user-select:all}.copy-button{background:#7c4b00;border:0;border-radius:8px;color:#fff;cursor:pointer;font:750 .82rem system-ui;padding:.55rem .8rem}.copy-button:hover{background:#593600}.copy-button:focus-visible{outline:3px solid #e1ad51;outline-offset:2px}.copy-status{display:block;font-size:.78rem;min-height:1.2em;margin-top:.3rem}.kind-banner.ready .kind-command-row,.kind-banner.ready .copy-status{display:none}
  ...
  <section class="panel bundle-panel" aria-labelledby="bundles-title">
    ...
    <div id="kind-banner" class="kind-banner" role="status">
      ...
      <div id="kind-setup" class="kind-setup">
        <p>Danach diesen Einzeiler im Projektverzeichnis ausführen:</p>
        <div class="kind-command-row">
          <code id="kubernetes-command" class="kind-command">docker compose down &amp;&amp; kubectl config use-context docker-desktop &amp;&amp; kubectl apply -k 'https://github.com/Student007/demoscale//kubernetes?ref=1.2.9' &amp;&amp; kubectl -n demoscale wait --for=condition=Available deployment --all --timeout=180s &amp;&amp; kubectl -n demoscale get pods -o wide &amp;&amp; kubectl -n demoscale port-forward service/dashboard 8080:8080</code>
...
async function copyKubernetesCommand(){
  const command=document.querySelector('#kubernetes-command').textContent;
  const status=document.querySelector('#copy-status');
  try{
    await navigator.clipboard.writeText(command);
    status.textContent='Einzeiler kopiert. Jetzt im Terminal im Projektverzeichnis einfügen.';
  }catch(error){
    status.textContent='Automatisches Kopieren nicht möglich. Den Einzeiler markieren und manuell kopieren.';
    console.error(error);
  }
}
