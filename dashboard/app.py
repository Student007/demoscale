...
BUNDLE_IMAGE = os.environ.get("BUNDLE_IMAGE", "danbu/demoscale:demoscale-bundle-task-1.2.10")
...
.kind-banner strong{display:block}.kind-banner p{margin:.3rem 0}.kind-setup[hidden],.kind-command[hidden]{display:none!important}.shell-tabs{display:flex;flex-wrap:wrap;gap:.4rem;margin-top:.7rem}.shell-tab{background:#fff7e8;border:1px solid #c9973f;border-radius:999px;color:#6c4100;cursor:pointer;font:750 .78rem system-ui;padding:.4rem .7rem}.shell-tab.active,.shell-tab[aria-selected="true"]{background:#7c4b00;color:#fff}.kind-command-row{align-items:stretch;display:flex;gap:.5rem;margin-top:.7rem}.kind-command{background:#fff;border:1px solid #d4a653;border-radius:8px;color:#503000;display:block;flex:1;font:650 .78rem/1.45 ui-monospace,SFMono-Regular,Menlo,monospace;overflow-wrap:anywhere;padding:.65rem .75rem;white-space:pre-wrap}.copy-button{background:#7c4b00;border:0;border-radius:8px;color:#fff;cursor:pointer;font:750 .82rem system-ui;padding:.55rem .8rem}.copy-button:hover{background:#593600}.copy-button:focus-visible,.shell-tab:focus-visible{outline:3px solid #e1ad51;outline-offset:2px}.copy-status{display:block;font-size:.78rem;min-height:1.2em;margin-top:.3rem}.kind-banner.ready .kind-command-row,.kind-banner.ready .copy-status{display:none}
...
<div id="bundles-view" class="view-panel" hidden>
  <section class="panel bundle-panel" aria-labelledby="bundles-title">
    ...
    <div id="kind-banner" class="kind-banner" role="status">
      <strong id="kind-banner-title">Kubernetes/KIND ist für diese Dashboard-Instanz nicht aktiv.</strong>
      <p id="kind-banner-note">KIND einmalig unter Docker Desktop → Einstellungen/Settings → Kubernetes aktivieren und vier Nodes auswählen.</p>
      <div id="kind-setup" class="kind-setup">
        <p id="shell-instruction">Diese Befehle unter macOS oder Linux einfügen (Git wird nicht benötigt):</p>
        <div class="shell-tabs" role="tablist" aria-label="Terminal auswählen">
          <button class="shell-tab active" data-shell="unix" role="tab" aria-selected="true" type="button">macOS / Linux</button>
          <button class="shell-tab" data-shell="powershell" role="tab" aria-selected="false" type="button">Windows PowerShell</button>
        </div>
        <div class="kind-command-row">
          <code class="kind-command" data-shell-command="unix">docker compose down &amp;&amp; \
...
kubectl -n demoscale port-forward service/dashboard 8080:8080</code>
          <code class="kind-command" data-shell-command="powershell" hidden>docker compose down
...
kubectl -n demoscale get pods -o wide
kubectl -n demoscale port-forward service/dashboard 8080:8080</code>
          <button id="copy-kubernetes-command" class="copy-button" type="button">Kopieren</button>
        </div>
...
function renderBundles(state){
  ...
  document.querySelector('#kind-banner-title').textContent=enabled?'Kubernetes/KIND aktiv':'Kubernetes/KIND ist für diese Dashboard-Instanz nicht aktiv.';
  document.querySelector('#kind-banner-note').textContent=enabled?'Kind-Modus erkannt. Die Bundle-Jobs können als Multi-Container-Pods gestartet werden.':'KIND einmalig unter Docker Desktop → Einstellungen/Settings → Kubernetes aktivieren und vier Nodes auswählen.';
  document.querySelector('#kind-setup').hidden=enabled;
...
async function copyKubernetesCommand(){
  const command=document.querySelector('[data-shell-command]:not([hidden])').textContent;
  const status=document.querySelector('#copy-status');
  try{
    await navigator.clipboard.writeText(command);
    const shell=document.querySelector('[data-shell].active').dataset.shell;
    status.textContent=(shell==='powershell'?'PowerShell':'Terminal')+'-Befehle kopiert. Jetzt einfügen.';
  }catch(error){
    status.textContent='Automatisches Kopieren nicht möglich. Die Befehle markieren und manuell kopieren.';
    console.error(error);
  }
}
function selectShell(shell){
  document.querySelectorAll('[data-shell]').forEach(button=>{const active=button.dataset.shell===shell;button.classList.toggle('active',active);button.setAttribute('aria-selected',String(active));});
  document.querySelectorAll('[data-shell-command]').forEach(command=>command.hidden=command.dataset.shellCommand!==shell);
  document.querySelector('#shell-instruction').textContent=shell==='powershell'?'Diese Befehle in Windows PowerShell einfügen (Git wird nicht benötigt):':'Diese Befehle unter macOS oder Linux einfügen (Git wird nicht benötigt):';
...
document.querySelector('#copy-kubernetes-command').addEventListener('click',copyKubernetesCommand);
document.querySelectorAll('[data-shell]').forEach(button=>button.addEventListener('click',()=>selectShell(button.dataset.shell)));
document.querySelectorAll('[data-view]').forEach(button=>button.addEventListener('click',()=>setView(button.dataset.view)));
selectShell(navigator.userAgent.includes('Windows')?'powershell':'unix');
